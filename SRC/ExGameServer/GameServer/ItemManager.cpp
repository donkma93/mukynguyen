...
bool CItemManager::CheckItemMoveToTrade(LPOBJ lpObj, CItem* lpItem, BYTE TargetFlag)
{
	...
	if (lpItem->IsPentagramItem() != 0) {
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 2, 0, gMessageNew.GetMessage(499));
			return 0;
		}
	}
...
void CItemManager::CGItemDropRecv(PMSG_ITEM_DROP_RECV* lpMsg, int aIndex)
{
	...
	else if (lpItem->m_Index == GET_ITEM(13, 20) && lpItem->m_Level == 1) // Starter Ring 40
	{
		if (lpObj->Level < 40)
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(272));
			DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
			return;
		}
	...
	else if (lpItem->m_Index == GET_ITEM(13, 20) && lpItem->m_Level == 2) // Starter Ring 80
	{
		if (lpObj->Level < 80)
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(274));
			DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
			return;
		}
...
void CItemManager::CGItemBuyRecv(PMSG_ITEM_BUY_RECV* lpMsg, int aIndex, int ok)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(634));
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}

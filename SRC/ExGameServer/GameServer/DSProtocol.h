...
// CMS GS item delivery queue. The DataServer returns one packet per pending item row
// after a character has fully loaded, so the normal item-serial path stays intact.
struct SDHP_ADMIN_ITEM_DELIVERY_SEND
{
	PSBMSG_HEAD header; // C1:0F:32
	WORD index;
	char account[11];
	char name[11];
};

struct SDHP_ADMIN_ITEM_DELIVERY_RECV
{
	PSBMSG_HEAD header; // C1:0F:32
	WORD index;
	char account[11];

...
bool CCustomAttack::CommandCustomAttack(LPOBJ lpObj, char* arg)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(691));
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(563));
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(692));
		return 0;
	}
	...
	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(695));

	if (this->m_CustomAttackTime[lpObj->AccountLevel] > 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(693), this->m_CustomAttackTime[lpObj->AccountLevel]);
	}
	return 1;
}
...
void CCustomAttack::OnAttackClose(LPOBJ lpObj)
{
	if (lpObj->AttackCustom != 0)
	{
		...
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(690));
...
void CCustomAttack::OnAttackMonsterAndMsgProc(LPOBJ lpObj)
{
	...
	if (lpObj->AttackCustomTime > 0)
	{
		if ((--lpObj->AttackCustomTime) == 0)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(694));
			this->OnAttackClose(lpObj);
			return;
		}
	}

...
void CReiDoMU::CommandReiDoMU(LPOBJ lpObj, char* arg)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(307));
		return;
	}

	if (lpGuildInfo->TotalCount < gServerInfo.m_ReiDoMUGuildMinMembers) //Verifica quantidade de membros na guild
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(309), gServerInfo.m_ReiDoMUGuildMinMembers);
		return;
	}
	...
	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(303), lpObj->GuildName);

	memcpy(this->m_GuildOwner, lpObj->GuildName, sizeof(this->m_GuildOwner));

	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(310));

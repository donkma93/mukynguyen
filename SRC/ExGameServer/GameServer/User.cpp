...
void gObjFirstProc()
{
	...
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		...
		if (Coin1 > 0 || Coin2 > 0 || Coin3 > 0)
			GDSetCoinSend(lpObj->Index, Coin1, Coin2, Coin3, 0, 0, "OnlineReward");
	}
}
...
void gObjCloseProc()
{
	if (gCloseMsg != 0)
	{
		if ((--gCloseMsgTime) <= 1)
		{
			if (gCloseMsgTime == 1)
			{
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(480));
				LogAdd(LOG_RED, gMessageNew.GetMessage(480));
			}
		}
		else
		{
			...
			{
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(481), gCloseMsgTime);
				LogAdd(LOG_RED, gMessageNew.GetMessage(481), gCloseMsgTime);
			}
		}
...
short gObjAdd(SOCKET socket, char* IpAddress, int aIndex)
{
	...
	//LogAddConnect(LOG_GREEN,"AddAccount: %s - IP: %s", lpObj->Account, lpObj->IpAddr);
...
{
	// Không dùng interface hoặc là Chaos Box thì bỏ qua
	...
	// Chưa quá 5 giây kể từ lần cập nhật cuối thì bỏ qua
	...
	{
		// state = 0 => đang chờ accept
		if (lpObj->Interface.state == 0)
		{
			// Kiểm tra đối tác trade
			if (OBJECT_RANGE(lpObj->TargetNumber) != 0)
			{
				if (gObj[lpObj->TargetNumber].Interface.type == INTERFACE_TRADE)
				{
					// 🔒 Chống dupe: commit inventory của đối tác trước khi reset
					gObjInventoryCommit(lpObj->TargetNumber);
					...
					// Thông báo hủy trade cho đối tác
					gTrade.GCTradeResultSend(lpObj->TargetNumber, 3);
				}
			}

			// 🔒 Chống dupe: commit inventory của chính mình
	...
	// Cập nhật thời gian kiểm tra để tiếp tục đếm lần sau
...
void gObjUserDie(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	if (gServerInfo.m_DieUserSwitch == 1 && lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER)
	{
		...
		{
			PostMessagePK(lpTarget->Name, gMessageNew.GetMessage(315), lpObj->Name);
		}
	}
	//Die System Monster
	if (gServerInfo.m_DieMonsterSwitch == 1 && lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_MONSTER)
	{
		PostMessagePK(lpTarget->Name, gMessageNew.GetMessage(316), lpObj->Name);
	}
...
BOOL gObjMoveGate(int aIndex, int gate)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(253));
		goto ERROR_JUMP;
	}
#endif
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(266));
		goto ERROR_JUMP;
	}
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(276));
		goto ERROR_JUMP;
	}
	...
	if (map == MAP_RAKLION2)
	{
		...
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(277));
			goto ERROR_JUMP;
		}
		else
		{
...
void gObjSkillUseProc(LPOBJ lpObj)
{
	...
	if (lpObj->Type == OBJECT_USER && lpObj->SkillSummonPartyTime > 0)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(273), lpObj->SkillSummonPartyTime);
		}
	}
...
bool gObjInventorySearchSerialNumber(LPOBJ lpObj, DWORD serial)
{
	...
	for (int n = 0; n < INVENTORY_SIZE; n++)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(29));
			...
			LogAdd(LOG_RED, "Dupe Serial: [%s][%s] %02X ", lpObj->Account, lpObj->Name, serial);
...
bool gObjWarehouseSearchSerialNumber(LPOBJ lpObj, DWORD serial)
{
	...
	for (int n = 0; n < WAREHOUSE_SIZE; n++)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(482));
			gObjUserKill(lpObj->Index);
			return 0;
		}
	}
	...
	for (n = 0; n < MAX_OBJECT; n++)
	{
			...
			if (lpObj->Type == OBJECT_USER)
			{
				...
				if (lpObj->ChatLimitTime > 0)
				{
					...
					if (lpObj->ChatLimitTimeSec > 60)
					{
						...
						{
							lpObj->ChatLimitTime = 0;
							gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(483));
						}
					}
				}
				...
				if (lpObj->LuckySpinTime > 0 && GetTickCount() - lpObj->LuckySpinTime > 5000)
				{
					...
					pMsg.header.set(0xF3, 0x31, sizeof(pMsg));
		...
		if (lpObj->Connected >= OBJECT_LOGGED && lpObj->Type == OBJECT_USER && lpObj->CloseCount > 0)
		{
			...
			else
			{
				...
				{
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(659), (lpObj->CloseCount - 1));
				}
				else if (lpObj->CloseType == 4)
				{
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(658), (lpObj->CloseCount - 1));
				}
				else
				{
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(484), (lpObj->CloseCount - 1));
				}
			}
			lpObj->CloseCount--;
		}
...
void gObjCheckSelfDefense(LPOBJ lpObj, int aTargetIndex)
{
	...
	wsprintf(szTemp, gMessageNew.GetMessage(485), lpObj->Name, gObj[aTargetIndex].Name);
...
void gObjTimeCheckSelfDefense(LPOBJ lpObj)
{
	...
	for (int n = 0; n < MAX_SELF_DEFENSE; n++)
	{
		if (lpObj->SelfDefense[n] >= 0)
		{
			...
			{
				wsprintf(szTemp, gMessageNew.GetMessage(486), lpObj->Name);
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, szTemp);

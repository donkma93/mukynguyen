...
void CPersonalShop::CGPShopOpenRecv(PMSG_PSHOP_OPEN_RECV* lpMsg,int aIndex)
{
	...
	//FIX Dupe: Chặn mở shop khi đang ở giao diện khác (NPC, Trade, Chaos...)
...
void CPersonalShop::CGPShopItemListRecv(PMSG_PSHOP_ITEM_LIST_RECV* lpMsg,int aIndex)
{
	...
	//FIX Dupe: Chặn list shop khi đang ở giao diện khác
	if (lpObj->Interface.use != 0 && lpObj->Interface.type != INTERFACE_PERSONAL_SHOP)
	{
		this->GCPShopItemListSend(aIndex, -1, 0, 0);
		return;
	}
...
void CPersonalShop::CGPShopBuyItemRecv(PMSG_PSHOP_BUY_ITEM_RECV* lpMsg, int aIndex)
{
	...
	//FIX Dupe: Chặn mua khi đang ở giao diện khác
	if (lpObj->Interface.use != 0 && lpObj->Interface.type != INTERFACE_PERSONAL_SHOP)
	{
	...
	if (result == 0xFF)
	{
		//FIX Dupe: Reset trạng thái để không kẹt giao dịch người bán
	...
	gLog.Output(LOG_TRADE, "[SellPesonalShopItem][%s][%s] - (Account: %s, Name: %s, Value: %d, Index: %04d, Level: %02d, Serial: %08X, Option1: %01d, Option2: %01d, Option3: %01d, NewOption: %03d, JewelOfHarmonyOption: %03d, ItemOptionEx: %03d, SocketOption: %03d, %03d, %03d, %03d, %03d)",
		lpTarget->Account, lpTarget->Name, lpObj->Account, lpObj->Name, lpTarget->Inventory[lpMsg->slot].m_PShopValue,
		lpTarget->Inventory[lpMsg->slot].m_Index, lpTarget->Inventory[lpMsg->slot].m_Level, lpTarget->Inventory[lpMsg->slot].m_Serial,
		lpTarget->Inventory[lpMsg->slot].m_Option1, lpTarget->Inventory[lpMsg->slot].m_Option2, lpTarget->Inventory[lpMsg->slot].m_Option3,
		lpTarget->Inventory[lpMsg->slot].m_NewOption, lpTarget->Inventory[lpMsg->slot].m_JewelOfHarmonyOption,
		lpTarget->Inventory[lpMsg->slot].m_ItemOptionEx, lpTarget->Inventory[lpMsg->slot].m_SocketOption[0],
		lpTarget->Inventory[lpMsg->slot].m_SocketOption[1], lpTarget->Inventory[lpMsg->slot].m_SocketOption[2],
		lpTarget->Inventory[lpMsg->slot].m_SocketOption[3], lpTarget->Inventory[lpMsg->slot].m_SocketOption[4]);

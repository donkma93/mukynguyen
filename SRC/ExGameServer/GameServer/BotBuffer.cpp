...
BOOL ObjBotBuffer::TradeOpen(int index, int nindex)
{
	...
	{
		gNotice.NewMessageDevTeam(lpObj->Index, gMessageNew.GetMessage(321), this->bot[number].Name);
		return 1;
	}
	if (this->bot[number].OnlyVip != 0 && lpObj->AccountLevel == 0)
	{
		gNotice.NewMessageDevTeam(gObj[nindex].Index, gMessageNew.GetMessage(322), this->bot[number].Name);
		return 1;
	}
	if ((DWORD)this->bot[number].Zen > gObj[index].Money)
	{
		gNotice.NewMessageDevTeam(gObj[index].Index, gMessageNew.GetMessage(323), this->bot[number].Name, this->bot[number].Zen);
		return 1;
	}
	if (this->bot[number].PCPoints > gObj[index].PCPoint)
	{
		gNotice.NewMessageDevTeam(gObj[index].Index, gMessageNew.GetMessage(324), this->bot[number].Name, this->bot[number].PCPoints);
		return 1;
	}
	if (this->bot[number].ActiveGensFamily == 1)
	{
		...
		{
			gNotice.NewMessageDevTeam(gObj[index].Index, gMessageNew.GetMessage(325), this->bot[number].Name);
			return 1;
		}
		if (this->bot[number].GensFamily != GENS_FAMILY_DUPRIAN && gObj[index].GensFamily == GENS_FAMILY_NONE)
		{
			gNotice.NewMessageDevTeam(gObj[index].Index, gMessageNew.GetMessage(326), this->bot[number].Name);
			return 1;
		}
		if (this->bot[number].GensFamily == GENS_FAMILY_VARNERT && gObj[index].GensFamily == GENS_FAMILY_DUPRIAN)
		{
			gNotice.NewMessageDevTeam(gObj[index].Index, gMessageNew.GetMessage(327), this->bot[number].Name);
			return 1;
		}
		if (this->bot[number].GensFamily == GENS_FAMILY_DUPRIAN && gObj[index].GensFamily == GENS_FAMILY_VARNERT)
		{
			gNotice.NewMessageDevTeam(gObj[index].Index, gMessageNew.GetMessage(328), this->bot[number].Name);
			return 1;
		}
		if (this->bot[number].ContributionGens > gObj[index].GensContribution)
		{
			gNotice.NewMessageDevTeam(gObj[index].Index, gMessageNew.GetMessage(329), this->bot[number].Name, this->bot[number].ContributionGens);
			return 1;
		}
	}
	...
	for (int x = 0; x < this->bot[number].SkillCount; x++)
	{
		...
		case SKILL_MANA_SHIELD:
		{
			...
			gSkillManager.GCSkillAttackSend(lpBot, SKILL_MANA_SHIELD, index, 1);
	...
	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(333), this->bot[number].Name);
	return 1;
}

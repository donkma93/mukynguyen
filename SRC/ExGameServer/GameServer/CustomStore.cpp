...
bool CCustomStore::CommandCustomStoreOffline(LPOBJ lpObj, char* arg)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(687));
		return 0;
	}

	if (gMap[lpObj->Map].CheckAttr(lpObj->X, lpObj->Y, 1) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(679));
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(65));
		return 0;
	}
...
bool CCustomStore::OpenCustomStore(LPOBJ lpObj, int type)
{
	...
	//FIX Dupe: Chặn mở custom store khi đang mở interface khác
	if (lpObj->Interface.type != INTERFACE_NONE && lpObj->Interface.type != INTERFACE_PERSONAL_SHOP)
	{
		return 0;
	}
	if (lpObj->Interface.use != 0)
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(680));
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(688), this->m_CustomStoreTime[lpObj->AccountLevel]);
	}
...
void CCustomStore::OnPShopSecondProc(LPOBJ lpObj)
{
	...
	if (lpObj->PShopCustomTime > 0)
	{
		if ((--lpObj->PShopCustomTime) == 0)
		{
			...
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(689));
			return;
		}
	}
}
...
void CCustomStore::OnPShopItemList(LPOBJ lpObj, LPOBJ lpTarget)
{
	if (lpTarget->PShopCustom != 0)
	{
		...
		{
		case 0:
			gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(681));
			break;
		case 1:
			gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(682));
			break;
		case 2:
			gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(683));
			break;
		case 3:
			gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(684));
			break;
		case 4:
			gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(685));
			break;
		case 5:
			gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(686));
			break;
		}
	}
}
...
bool CCustomStore::OnPShopBuyItemRecv(PMSG_PSHOP_BUY_ITEM_RECV* lpMsg, int aIndex)
{
	...
	//FIX Dupe: Chặn mua từ custom store khi đang mở interface khác
	if (lpObj->Interface.use != 0 && lpObj->Interface.type != INTERFACE_PERSONAL_SHOP)
	{
		gPersonalShop.GCPShopBuyItemSend(aIndex, MAKE_NUMBERW(lpMsg->index[0], lpMsg->index[1]), 0, 6);
		return 1;
	}
	...
	if (result == 0xFF)
	{
		//FIX Dupe: Reset trạng thái giao dịch khi thất bại
...
{
	//FIX Dupe: Kiểm tra người mua còn online
	...
	//FIX Dupe: Chặn khi buyer đang ở interface khác
	if (lpObj->Interface.use != 0 && lpObj->Interface.type != INTERFACE_PERSONAL_SHOP)
	{
	...
	gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, PShopWCCValue, PShopWCPValue, PShopWGPValue, 0, 0);

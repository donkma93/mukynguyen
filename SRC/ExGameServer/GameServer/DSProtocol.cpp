...
void DataServerProtocolCore(BYTE head,BYTE* lpMsg,int size)
{
	switch(head)
	{
		...
		case 0x0F:
			switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
			{
                ...
                case 0x09:
                    ...
                    break;
				case 0x32:
					DGAdminItemDeliveryRecv((SDHP_ADMIN_ITEM_DELIVERY_RECV*)lpMsg);
					break;
			}
			break;
		case 0x10:
		...
		switch (((lpMsg[0] == 0xC1) ? lpMsg[3] : lpMsg[4]))
		{
			...
			case 0xF4:	gRanking.RecvDSTG((DGCharTopTG*)lpMsg);				break;
			case 0x05:	gRanking.RecvDSPK((DGCharTopPK*)lpMsg);				break;
			case 0x06:	gRanking.RecvDSMT((DGCharTopMT*)lpMsg);				break;
			case 0x07:	gRanking.RecvDSWQ((DGCharTopWQ*)lpMsg);				break;
		}
		break;
		...
		switch (((lpMsg[0] == 0xC1) ? lpMsg[3] : lpMsg[4]))
		{
			case 0x40:	gCustomReadGuild.RecvInfoCharTop((ReadFromLeach*)lpMsg);	break;
		}
		break;
	}
}
...
void DGCharacterListRecv(SDHP_CHARACTER_LIST_RECV* lpMsg)
{
	...
	gHelper.GDHelperDataSend(lpMsg->index);
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg)
{
	...
	//LogAdd(LOG_RED, "0xF3, 0x03 PMSG_CHARACTER_INFO_SEND size: %d bytes", sizeof(PMSG_CHARACTER_INFO_SEND));
	...
	DataSend(lpObj->Index, (BYTE*)&pMsg, pMsg.header.size); // 0xF3, 0x03 DGCharacterInfoRecv Rejoin
	...
	// The CMS queue is read only after inventory and character data are ready.
	GDAdminItemDeliverySend(lpObj->Index);
...
#if(MEMBER_ONLINE)
	//post mem online
	if (gServerInfo.MemberOnlineSubSwitch == 1) {
		gObjCountProc();
		int totalOnline = max(1, gObjTotalUser);
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(354), totalOnline, gServerInfo.m_ServerMaxUserNumber);
	}
#endif
	...
	if (lpObj->Authority == 32)
	{
		...
		{
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(8), lpObj->Name);
		}
	}
	else
	{
		...
		{
			PostMessageUserON(lpObj->Name, gMessageNew.GetMessage(9));
		}
	}
	...
	{
		if (gServerInfo.m_ServerAdmin != 0 && gGameMaster.CheckGameMasterLevel(lpObj, 1) == 0)
		{
			lpObj->CloseCount = 6;
			lpObj->CloseType = 3;
			return;
...
void DGAdminItemDeliveryRecv(SDHP_ADMIN_ITEM_DELIVERY_RECV* lpMsg)
{
	if (gObjIsAccountValid(lpMsg->index, lpMsg->account) == 0)
	{
		return;
	...
	LPOBJ lpObj = &gObj[lpMsg->index];
	if (strcmp(lpObj->Name, lpMsg->name) != 0)
	{
		return;
	}

	BYTE socketOption[MAX_SOCKET_OPTION] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
	for (int n = 0; n < lpMsg->SocketCount && n < MAX_SOCKET_OPTION; n++)
	{
		socketOption[n] = 0xFE;
	}

	int quantity = ((lpMsg->Quantity < 1) ? 1 : ((lpMsg->Quantity > 25) ? 25 : lpMsg->Quantity));
	for (int n = 0; n < quantity; n++)
	{
		GDCreateItemSend(lpObj->Index, 0xEB, 0, 0, lpMsg->ItemIndex,
			lpMsg->Level, lpMsg->Dur, lpMsg->Option1, lpMsg->Option2,
			lpMsg->Option3, -1, lpMsg->NewOption, lpMsg->SetOption,
			0, 0, socketOption, 0xFF,
			((lpMsg->Duration > 0) ? ((DWORD)time(0) + lpMsg->Duration) : 0));
	}

	gLog.Output(LOG_COMMAND, "[CMSItemDelivery][%s][%s] - Delivery: %d, Item: %d, Amount: %d",
		lpObj->Account, lpObj->Name, lpMsg->deliveryId, lpMsg->ItemIndex, quantity);
}
...
void GDAdminItemDeliverySend(int aIndex)
{
	if (gObjIsAccountValid(aIndex, gObj[aIndex].Account) == 0)
	{
		return;
	...
	SDHP_ADMIN_ITEM_DELIVERY_SEND pMsg;
	pMsg.header.set(0x0F, 0x32, sizeof(pMsg));
	pMsg.index = aIndex;
	memcpy(pMsg.account, gObj[aIndex].Account, sizeof(pMsg.account));
...
void GDSetCoinSend(int aIndex, int value1, int value2, int value3, int value4, int value5, char* LogName)
{
	...
	if (value1 > 0)
	{
		lpObj->ThisCoin[0] += value1;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(10), value1);
	}
	if (value2 > 0)
	{
		lpObj->ThisCoin[1] += value2;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(11), value2);
	}
	if (value3 > 0)
	{
		lpObj->ThisCoin[2] += value3;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(12), value3);
	}
	if (value4 > 0)
	{
		lpObj->ThisCoin[3] += value4;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(44), value4);
	}

	if (value5 > 0)
	{
		lpObj->ThisCoin[4] += value5;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(45), value5);
	}

	if (value1 < 0)
	{
		lpObj->ThisCoin[0] += value1;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(13), value1, "WC");
	}
	if (value2 < 0)
	{
		lpObj->ThisCoin[1] += value2;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(13), value2, "WP");
	}
	if (value3 < 0)
	{
		lpObj->ThisCoin[2] += value3;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(13), value3, "GP");
	}
	if (value4 < 0)
	{
		lpObj->ThisCoin[3] += value4;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(13), value4, "RD");
	}
	if (value5 < 0)
	{
		lpObj->ThisCoin[4] += value5;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(13), value5, "ATM");
	}
	gCashShop.CGCashShopPointRecv(lpObj->Index);

	LogAdd(LOG_RED, "[%s] LogCoin(%s) WC : %d, WP : %d, GP : %d, Rud: %d, ATM: %d", lpObj->Account, lpObj->Name, value1, value2, value3, value4, value5);
}

...
bool CMonter::DelMember(int aIndex)
{
	for (int i = 0; i < TOPDAME; i++)
	{
		...
		{
			int indexQuai = this->Player[i].IndexQuai;

			this->Player[i].Index = -1;
			this->Player[i].Dmg = 0;
			this->Player[i].Live = false;
			this->Player[i].Rank = 0;
			this->Player[i].Regen = 0;
			this->Player[i].IndexQuai = -1;
			this->Player[i].LastHitTick = 0;
			this->Player[i].DeadTick = 0;

			if (OBJECT_RANGE(indexQuai))
			{
				LPOBJ lpMonster = &gObj[indexQuai];

				for (int h = 0; h < MAX_HIT_DAMAGE; h++)
				{
					if (lpMonster->HitDamage[h].index == aIndex)
					{
						lpMonster->HitDamage[h].index = -1;
						lpMonster->HitDamage[h].indexquai = -1;
						lpMonster->HitDamage[h].damage = 0;
						lpMonster->HitDamage[h].time = 0;

						if (lpMonster->HitDamageCount > 0)
						{
							lpMonster->HitDamageCount--;
						}
					}
				}
			}
...
void CMonter::SendBXH(LPOBJ lpObj, int aIndex)
{
	...
	for (int n = 0; n < 10; n++)
	{
		...
		if (this->Player[this->mIndexRank[n]].Rank > 0 && this->Player[this->mIndexRank[n]].Rank <= 10)
		{
			...
			info.Kill = this->Player[this->mIndexRank[n]].Dmg;
			pMsg.count++;
...
void CMonter::CheckTimeoutMember()
{
	...
	for (int i = 0; i < TOPDAME; i++)
	{
		...
		int index = this->Player[i].Index;


		if ((now - this->Player[i].LastHitTick) > gServerInfo.m_RenderDameBossDelay * 1000)
		{
			this->DelMember(index);
			continue;
		}


		if (gObjIsConnected(index) && gObj[index].Live == false)
		{
			if (this->Player[i].DeadTick == 0)
			{
				this->Player[i].DeadTick = now;
			}
			else if ((now - this->Player[i].DeadTick) > gServerInfo.m_RenderDameBossClear * 1000)
			{
				this->DelMember(index);
			}
		}
...
void CMonter::AddPlayerPoint(int aIndex, QWORD damage, int Quai)
{
	for (int i = 0; i < TOPDAME; i++)
	{
		if (this->Player[i].Index == aIndex)
		{
			if (gObjIsConnectedGP(this->Player[i].Index) != 0)
			{
				...
				this->Player[i].LastHitTick = GetTickCount();
				this->Player[i].IndexQuai = Quai;

				if (gObj[aIndex].Live == false)
				{
					this->Player[i].DeadTick = GetTickCount();
				}
				else
				{
					this->Player[i].DeadTick = 0;
...
void gObjMonsterDieGiveItem(LPOBJ lpObj,LPOBJ lpTarget)
{
	...
	if (gCastleStartGuild.MonsterDie(lpObj->Index, lpTarget->Index))
	{
		return;
	}
...
bool gObjMonsterMoveCheck(LPOBJ lpObj,int tx,int ty)
{
	...
	else
	{
		if (lpObj->Class == 249 || lpObj->Class == 247)	// Guard Message
		{
			if (!(GetLargeRand() % 4))
			{
				GCChatTargetSend(lpObj, lpObj->Index, gMessageNew.GetMessage(28));
			}
		}
...
int gObjMonsterDelHitDamageUser(LPOBJ lpObj)
{
	...
	for(int n=0;n < MAX_HIT_DAMAGE;n++)
	{
		...
		else if((GetTickCount()-lpObj->HitDamage[n].time) > gServerInfo.m_RenderDameBossDelay * 1000)
		{
			result = 1;
		}
...
{
	int TopHitDamageUser = -1; // ID của người chơi gây sát thương cao nhất (ban đầu đặt là -1, nghĩa là chưa có ai)
	float TopHitDamage = 0; // Sát thương cao nhất gây ra (mặc định là 0)

	int PartyTopHitDamageUser = -1; // ID của người chơi trong nhóm (party) gây sát thương cao nhất
	float PartyTopHitDamage = 0; // Sát thương cao nhất do người chơi trong nhóm gây ra

	if (gServerInfo.m_BossDameCuoi) // Nếu cấu hình được bật
	{
		return lpObj->LastAttackerID; // Trả về ID người tấn công cuối cùng
	}
	...
	for(int n=0;n < MAX_HIT_DAMAGE;n++)
	{
		...
		if((GetTickCount()-lpObj->HitDamage[n].time) > gServerInfo.m_RenderDameBossDelay * 1000)
		{
			continue;
		}
...
void gObjMonsterAttack(LPOBJ lpObj,LPOBJ lpTarget)
{
	...
	else if(lpObj->Class == 529 || lpObj->Class == 530 || lpObj->Class == 533 || lpObj->Class == 534 || lpObj->Class == 535 || lpObj->Class == 536 || lpObj->Class == 537 || lpObj->Class == 538 || lpObj->Class == 539|| lpObj->Class == 580)
	{
...
void gObjMonsterProcess(LPOBJ lpObj)
{
	...
	//if (lpObj->Type == OBJECT_MONSTER && lpObj->Live)
	//{
	//	gMonter.CheckTimeoutMember();
	//}

	CUSTOM_MONSTER_INFO CustomMonsterInfo;
	if (gCustomMonster.GetCustomMonsterDrawBXH(lpObj->Class, lpObj->Map, &CustomMonsterInfo))
	{
		gMonter.CheckTimeoutMember();

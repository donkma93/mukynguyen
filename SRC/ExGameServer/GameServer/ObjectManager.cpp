...
void CObjectManager::ObjectSetStateProc()
{
	for (int n = 0; n < MAX_OBJECT; n++)
	{
		...
		if (lpObj->State == 1/*OBJECT_CREATE*/)
		{
...
void CObjectManager::CharacterGameCloseSet(int aIndex, int type)
{
	...
	if (lpObj->Connected == OBJECT_ONLINE)
	{
		...
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(291));
			return;
		}

		if (lpObj->Interface.use != 0 && lpObj->Interface.type == INTERFACE_WAREHOUSE)
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(292));
			return;
		}

		if (lpObj->Interface.use != 0 && lpObj->Interface.type == INTERFACE_CHAOS_BOX)
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(293));
			return;
		}

		if (lpObj->Interface.use != 0 && lpObj->Interface.type == INTERFACE_TRAINER)
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(294));
			return;
		}

		if ((GetTickCount() - lpObj->MySelfDefenseTime) < 30000)
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(295));
			return;
		}

		//- Sửa lỗi Đổi nhân vật khi chết  [FIX2]
		if (lpObj->DieRegen != 0)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Bạn Không Có Quyền Sử Dụng!");
			return;
		}
...
bool CObjectManager::CharacterLevelUp(LPOBJ lpObj, DWORD AddExperience, int MaxLevelUp, int ExperienceType)
{
	if (gMasterSkillTree.CheckMasterLevel(lpObj) == 0)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(296));
			return 1;
		}
	...
	else
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(297));
			return 1;
		}
...
bool CObjectManager::CharacterLevelUpPointAdd(LPOBJ lpObj, int type, int amount)
{
	...
	if (stat == &lpObj->Leadership && ((*stat) + amount) > gServerInfo.m_MaxStatPointCMD[lpObj->AccountLevel]) {
		...
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(278), gServerInfo.m_MaxStatPointCMD[lpObj->AccountLevel]);
		return 0;
	}
	else if (((*stat) + amount) > gServerInfo.m_MaxStatPoint[lpObj->AccountLevel])
	{

		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(278), gServerInfo.m_MaxStatPoint[lpObj->AccountLevel]);
		return 0;
	}
...
bool CObjectManager::CharacterUseTalismanOfResurrection(LPOBJ lpObj, int SourceSlot)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(249));
		return 0;
	}
...
void CObjectManager::CharacterCalcAttribute(int aIndex)
{
	...
	while (change != 0)
	{
		...
		gDanhHieu.AddOption(lpObj, 1);
		gQuanHam.AddOption(lpObj, 1);
		gTuLuyen.AddOption(lpObj, 1);

		gHonHoan.AddOption(lpObj, 1);
	...
	gDanhHieu.AddOption(lpObj, 0);
	gQuanHam.AddOption(lpObj, 0);
	gTuLuyen.AddOption(lpObj, 0);
	gHonHoan.AddOption(lpObj, 0);
		...
		if (lpTarget->Type == OBJECT_MONSTER)
		{
			...
			if (lpObj->CustomQuestMonsterIndex >= 0)
			{
				if (lpObj->CustomQuestMonsterIndex == lpTarget->Class && lpObj->CustomQuestMonsterQtd != 0)
				{
					...
					{
						gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(655));
					}
					else
					{
						gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(654), lpObj->CustomQuestMonsterQtd);
					}
				}
			}
			...
			if (lpObj->CustomNpcQuestMonsterIndex >= 0)
			{
				if (lpObj->CustomNpcQuestMonsterIndex == lpTarget->Class && lpObj->CustomNpcQuestMonsterQtd != 0)
				{
					...
					{
						gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(654), lpObj->CustomNpcQuestMonsterQtd);
					}
				}
			}

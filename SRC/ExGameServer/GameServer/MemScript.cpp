...
bool CMemScript::SetBuffer(const std::string& path)
{
...
bool CMemScript::GetBuffer(std::vector<char>& buff, DWORD& size)
{
	if (this->m_buff == nullptr)
	{
		this->SetLastError(3);
...
const wchar_t* CMemScript::GetAsWideString()
{
	static wchar_t wStr[256];
	MultiByteToWideChar(CP_UTF8, 0, this->GetAsString(), -1, wStr, 256);
	return wStr;
}
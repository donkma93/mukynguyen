...
void ProtocolCore(BYTE head, BYTE* lpMsg, int size, int aIndex, int encrypt, int serial)
{
	...
	switch (((lpMsg[0] == 0xC1) ? lpMsg[3] : lpMsg[4]))
	{
		case 0x01:
		{
			...
			gHonHoan.SendInfoClient(aIndex);
		}
		break;
		case 0x02:
		{
		...
		case 0x05:
		{
			...
			if (!mlpMsg->ThaoTac) { return; }
			gDanhHieu.SendInfoClient(aIndex);
		}
		break;
		case 0x06:
		{
			SolvedDanhHieu* mlpMsg = (SolvedDanhHieu*)lpMsg;
			if (!mlpMsg->ThaoTac) { return; }
			gDanhHieu.RequestAccess(aIndex);
		}
		break;
		...
		case 0x07:
		{
			...
			if (!mlpMsg->ThaoTac) { return; }
			gQuanHam.SendInfoClient(aIndex);
		}
		break;
		case 0x08:
		{
		...
		case 0x09:
		{
			...
			if (!mlpMsg->ThaoTac) { return; }
			gTuLuyen.SendInfoClient(aIndex);
		}
		break;
		case 0x10:
		{
	...
	switch (lpMsg[3])
	{
		...
		case 0x09: gHidManager.CGHardwareIdRecv((PMSG_HARDWAREID_INFO_RECV*)lpMsg, aIndex);					break;
		...
		case 0xDA: g_ExWinQuestSystem.QuestChange(aIndex, (QUESTCHANGE_REQ*)lpMsg);							break;
		case 0xE7: gResetTable.SendRequestAuto((CGPACKET_AUTO*)lpMsg, aIndex);								break;
		case 0xE9: gResetTable.SendRequest(aIndex);															break;
	}
	break;
	...
	switch (lpMsg[3])
	{
		...
		case 0x0A: gCommandManager.CommandAddBank(&gObj[aIndex], "0");			break;
		case 0x0B: gCommandManager.CommandAddBank(&gObj[aIndex], "1");			break;
		case 0x0C: gCommandManager.CommandAddBank(&gObj[aIndex], "2");			break;
		case 0x0D: gCommandManager.CommandAddBank(&gObj[aIndex], "3");			break;
		case 0x0E: gCommandManager.CommandAddBank(&gObj[aIndex], "4");			break;
		case 0x0F: gCommandManager.CommandAddBank(&gObj[aIndex], "5");			break;
		case 0x10: gCommandManager.CommandAddBank(&gObj[aIndex], "6");			break;
		case 0x11: gCommandManager.CommandAddBank(&gObj[aIndex], "7");			break;
		case 0x12: gCommandManager.CommandAddBank(&gObj[aIndex], "8");			break;
		case 0x13: gCommandManager.CommandAddBank(&gObj[aIndex], "9");			break;
		...
		case 0x34:  gCustomAttack.CommandCustomAttack(&gObj[aIndex], "");	        break;
		case 0x35:  gCustomAttack.CommandCustomAttackOffline(&gObj[aIndex], "");	break;
	...
	case 0xFF:
		switch (((lpMsg[0] == 0xC1) ? lpMsg[3] : lpMsg[4]))
		{
			...
			case 0x0D:	g_ExWinQuestSystem.CG_AcceptQuest(aIndex, (CG_Accept_Done*)lpMsg);	break;
		}
	break;
	...
	switch (((lpMsg[0] == 0xC1) ? lpMsg[3] : lpMsg[4]))
	{
		...
		case 0xF4:	gRanking.SendUserTG(lpObj);			break;
		case 0x05:	gRanking.SendUserPK(lpObj);			break;
		case 0x06:	gRanking.SendUserMT(lpObj);			break;
		case 0x07:	gRanking.SendUserWQ(lpObj);			break;
	}
	break;
	...
	case 0xD3:
	{
		switch (((lpMsg[0] == 0xC1) ? lpMsg[3] : lpMsg[4]))
		{
			case 0x02:
			{
				RecvGetInfoChar(aIndex, (XULY_CGPACKET_VIEWITEM*)lpMsg);
			}
			break;
			...
			case 0x40: gCustomReadGuild.CGetInfoCharTop((RequestGuilCache*)lpMsg, aIndex);			break;
		}
	}
	break;
	}
...
void CGChatRecv(PMSG_CHAT_RECV* lpMsg, int aIndex)
{
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(279), lpObj->ChatLimitTime);
		return;
	}
...
void CGChatWhisperRecv(PMSG_CHAT_WHISPER_RECV* lpMsg, int aIndex)
{
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(300));
		return;
	}
...
void CGLevelUpPointRecv(PMSG_LEVEL_UP_POINT_RECV* lpMsg, int aIndex)
{
	...
	if (gObjectManager.CharacterLevelUpPointAdd(lpObj, lpMsg->type, 1) != 0)
	{
		...
		switch (lpMsg->type)
		{
			case 2:				pMsg.MaxLifeAndMana = GET_MAX_WORD_VALUE((lpObj->MaxLife + lpObj->AddLife));				break;
			case 3:				pMsg.MaxLifeAndMana = GET_MAX_WORD_VALUE((lpObj->MaxMana + lpObj->AddMana));				break;
		}
		...
		pMsg.ManaClassReg[2] = GET_MAX_WORD_VALUE(lpObj->ELFDamageMultiplierRate);      // ELF
		pMsg.ManaClassReg[3] = GET_MAX_WORD_VALUE(lpObj->MGDamageMultiplierRate);       // MG
		pMsg.ManaClassReg[4] = GET_MAX_WORD_VALUE(lpObj->DLDamageMultiplierRate);       // DL
		pMsg.ManaClassReg[5] = GET_MAX_WORD_VALUE(lpObj->SUDamageMultiplierRate);       // SU
		pMsg.ManaClassReg[6] = GET_MAX_WORD_VALUE(lpObj->RFDamageMultiplierRate[1]);    // RF A
		pMsg.ManaClassReg[7] = GET_MAX_WORD_VALUE(lpObj->RFDamageMultiplierRate[2]);    // RF B
		
		//LogAdd(LOG_RED, "0xF3, 0x06 PMSG_LEVEL_UP_POINT_SEND size: %d bytes", sizeof(PMSG_LEVEL_UP_POINT_SEND));
	}
	DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size); // 0xF3, 0x06 CGLevelUpPointRecv
}
...
void GCDamageSend(int aIndex, int bIndex, BYTE flag, int damage, int type, int ShieldDamage)
{
	...
	if (gObj[bIndex].Type == OBJECT_MONSTER)
	{
		pMsg.CurHP = (BYTE)((gObj[bIndex].Life * 100) / (gObj[bIndex].MaxLife + gObj[bIndex].AddLife));
		pMsg.Level = gObj[bIndex].Level;
		pMsg.Life = gObj[bIndex].Life;
	}
	pMsg.CurExp = gObj[bIndex].Experience;
	pMsg.MaxExp = gObj[bIndex].NextExperience;
...
void GCLevelUpSend(LPOBJ lpObj)
{
	...
	pMsg.FruitAddPoint		= (DWORD)(lpObj->FruitAddPoint);
	pMsg.MaxFruitAddPoint	= gFruit.GetMaxFruitPoint(lpObj);
	pMsg.FruitSubPoint		= (DWORD)(lpObj->FruitSubPoint);
	...
	//LogAdd(LOG_RED, "0xF3, 0x05 PMSG_LEVEL_UP_SEND size: %d bytes", sizeof(PMSG_LEVEL_UP_SEND));

	DataSend(lpObj->Index, (BYTE*)&pMsg, pMsg.header.size); // 0xF3, 0x05 GCLevelUpSend
...
void GCNewCharacterInfoSend(LPOBJ lpObj)
{
	...
	pMsg.ViewCurSD			= (DWORD)(lpObj->Shield);
	pMsg.ViewMaxSD			= (DWORD)(lpObj->MaxShield + lpObj->AddShield);
	pMsg.ViewStrength		= (DWORD)lpObj->Strength;
	pMsg.ViewDexterity		= (DWORD)lpObj->Dexterity;
	...
	//LogAdd(LOG_RED, "0xF3, 0xE0 PMSG_NEW_CHARACTER_INFO_SEND size: %d bytes", sizeof(PMSG_NEW_CHARACTER_INFO_SEND));

	DataSend(lpObj->Index, (BYTE*)&pMsg, pMsg.header.size); // 0xF3, 0xE0 GCNewCharacterInfoSend
}
...
void GCNewCharacterCalcSend(LPOBJ lpObj)
{
	...
	//LogAdd(LOG_RED, "0xF3, 0xE1 PMSG_NEW_CHARACTER_CALC_SEND size: %d bytes", sizeof(PMSG_NEW_CHARACTER_CALC_SEND));
...
void RecvGetInfoChar(int aIndex, XULY_CGPACKET_VIEWITEM* lpMsg)
{
	...
	pMsg.pPlayerInfo[0] = gObj[bIndex].Reset;
	pMsg.pPlayerInfo[1] = gObj[bIndex].LevelUpPoint + gObj[bIndex].Strength + gObj[bIndex].Dexterity + gObj[bIndex].Vitality + gObj[bIndex].Energy + gObj[bIndex].Leadership;
...
void CSEND_CHANGEPASS(CSENDGS_DOIMK* lpMsg, int aIndex)
{
	...
	if (lpObj->ThisCoin[0] < 10000)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Không đủ Wcoin chi phí là: %d WC", 10000);
		return;
	}
	...
	memcpy(pRequest.CODE_NEW, lpMsg->CODE_NEW, 7);
...
void DSSENDGS_DOIMK_KQ(DSGS_DOIMK_KQ* lpMsg)
{
	...
	GDSetCoinSend(lpObj->Index, -(10000), 0, 0, 0, 0, "DoiPass");
	if (lpMsg->result == 3)
	{

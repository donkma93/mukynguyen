...
void CReconnect::ReconnectDrawInterface()
{
	...
	float StartX = (640.f / 2.f) - (160.f / 2.f) + gPosWide.x_GetAddPos;
	...
	{
	case RECONNECT_PROGRESS_NONE:
		g_pRenderText->RenderText((int)StartX, (int)StartY, GlobalText[3895]/*"Reconnecting..."*/, 0, 0, RT3_WRITE_CENTER);
		break;
	case RECONNECT_PROGRESS_CONNECTED:
		g_pRenderText->RenderText((int)StartX, (int)StartY, GlobalText[3896], 0, 0, RT3_WRITE_CENTER);
		break;
	case RECONNECT_PROGRESS_JOINED:
		g_pRenderText->RenderText((int)StartX, (int)StartY, GlobalText[3897], 0, 0, RT3_WRITE_CENTER);
		break;
	case RECONNECT_PROGRESS_CHAR_LIST:
		g_pRenderText->RenderText((int)StartX, (int)StartY, GlobalText[3898], 0, 0, RT3_WRITE_CENTER);
		break;
	}
	DisableAlphaBlend();
}
...
{
	char szIp[16] = { 0 };
	if (s_Data.GameServerAddress[0] != 0)
	{
		strcpy_s(szIp, s_Data.GameServerAddress);
	}
	...
	if (s_Data.GameServerPort == 0)
	{
		s_Data.GameServerPort = 55901;
	}

	if (CreateSocket(szIp, s_Data.GameServerPort) != 0)
	{
		g_bGameServerConnected = TRUE;

		ReconnectSetInfo(RECONNECT_STATUS_RECONNECT, RECONNECT_PROGRESS_CONNECTED, 100, (gProtect ? gProtect->m_MainInfo.ReconnectTime : 30) * 1000);
	}
}
...
void CReconnect::ReconnectOnCloseSocket()
{
	...
	{
		ReconnectSetInfo(RECONNECT_STATUS_RECONNECT, RECONNECT_PROGRESS_NONE, 300, (gProtect ? gProtect->m_MainInfo.ReconnectTime : 30) * 1000);
	...
	if (s_Data.ReconnectProgress == RECONNECT_PROGRESS_CONNECTED)
	{
		if (s_Data.ReconnectAuthSend != 0)
		{
			if (result == 1)
			{
				...
				ReconnectSetInfo(RECONNECT_STATUS_RECONNECT, RECONNECT_PROGRESS_JOINED, 100, (gProtect ? gProtect->m_MainInfo.ReconnectTime : 30) * 1000);
			}
			else
			{
				if (result == 3)
				{
					ReconnectSetInfo(RECONNECT_STATUS_RECONNECT, RECONNECT_PROGRESS_CONNECTED, 1000, (gProtect ? gProtect->m_MainInfo.ReconnectTime : 30) * 1000);
					s_Data.ReconnectAuthSend = 0;
				}
				else
				{
...
void CReconnect::ReconnectOnCharacterList()
{
	if (s_Data.ReconnectProgress == RECONNECT_PROGRESS_JOINED)
	{
		...
		ReconnectSetInfo(RECONNECT_STATUS_RECONNECT, RECONNECT_PROGRESS_CHAR_LIST, 100, (gProtect ? gProtect->m_MainInfo.ReconnectTime : 30) * 1000);
	}
}

...
void CMemScript::SkipLine()
{
	char ch;
	while ((ch = this->GetChar()) != -1)
	{
		if (ch == '\n') break;
	}
}

int CMemScript::GetSection(const char* section)
{
	...
	while (true)
	{
		...
		if (_stricmp(this->GetString(), sectionName) == 0)
		{
			return 1;
		}
	}
...
eTokenResult CMemScript::GetTokenType() //ACuoi
{
	if ((GetTickCount() - this->m_tick) > 1000)
	{
		this->SetLastError(4);

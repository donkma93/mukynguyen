...
#include "ThangCuoi/CustomMonster.h"
...
void OpenPlayerTextures()
{
	...
	gLoadData.OpenTexture(MODEL_SHADOW_BODY, "Player\\");
	{
		...
		LoadBitmap("ACuoi\\Item\\KF_darklordwing4de.tga", BITMAP_START_SPK_BIT);
		LoadBitmap("ACuoi\\Item\\KF_darklordwing4dd.tga", BITMAP_START_SPK_BIT + 1);
	}
...
void OpenItems()
{
	...
	LoadBitmap("ACuoi\\Item\\KF_ragefighterwing4db.TGA", BITMAP_START_SPK_BIT + 2, GL_LINEAR, GL_REPEAT);
	LoadBitmap("ACuoi\\Item\\KF_Death_clka.TGA", BITMAP_START_SPK_BIT + 3, GL_LINEAR, GL_REPEAT);
	LoadBitmap("ACuoi\\Item\\KF_Death_clkb.TGA", BITMAP_START_SPK_BIT + 4, GL_LINEAR, GL_REPEAT);
	...
	for (std::map<int, CUSTOM_BOW_INFO>::iterator it = gCustomBow.m_CustomBowInfo.begin(); it != gCustomBow.m_CustomBowInfo.end(); it++)
	{
		...
		if (ItemInfo->Index != -1)
		{
			gLoadData.AccessModel(ItemInfo->ItemIndex + MODEL_ITEM, "Data\\ACuoi\\Item\\", ItemInfo->ModelName);
		}
	}
	...
	for (std::map<int, CUSTOM_RF_GLOVES>::iterator it = gCustomBattleGloves.m_CustomGlovesInfo.begin(); it != gCustomBattleGloves.m_CustomGlovesInfo.end(); it++)
	{
		...
		{
			gLoadData.AccessModel(it->second.ItemIndex + MODEL_ITEM, "Data\\ACuoi\\Item\\", it->second.ModelName, -1);
			gLoadData.AccessModel(it->second.TextureLeftIndex + MODEL_ITEM, "Data\\ACuoi\\Item\\", it->second.TextureLeftName, -1);
			gLoadData.AccessModel(it->second.TextureRightIndex + MODEL_ITEM, "Data\\ACuoi\\Item\\", it->second.TextureRightName, -1);
		}
	}
	...
	for (std::map<int, CUSTOM_WING_INFO>::iterator it = gCustomWing.m_CustomWingInfo.begin(); it != gCustomWing.m_CustomWingInfo.end(); it++)
	{
		...
		gLoadData.AccessModel((lpInfo->ItemIndex + MODEL_ITEM), "Data\\ACuoi\\Item\\", lpInfo->ModelName);
	...
	for (int n = 0; n < MAX_CUSTOM_JEWEL; n++)
	{
		...
		{
			gLoadData.AccessModel((gCustomJewel.m_CustomJewelInfo[n].ItemIndex + MODEL_ITEM), "Data\\ACuoi\\Item\\", gCustomJewel.m_CustomJewelInfo[n].ModelName);
		}
	}
}
...
//---------------------------------------------------------------------------------
	...
	for (std::map<int, CUSTOM_BOW_INFO>::iterator it = gCustomBow.m_CustomBowInfo.begin(); it != gCustomBow.m_CustomBowInfo.end(); it++)
	{
		...
		if (ItemInfo->Index != -1)
		{
			gLoadData.OpenTexture(ItemInfo->ItemIndex + MODEL_ITEM, "ACuoi\\Item\\");
		}
	}
	...
	for (std::map<int, CUSTOM_RF_GLOVES>::iterator it = gCustomBattleGloves.m_CustomGlovesInfo.begin(); it != gCustomBattleGloves.m_CustomGlovesInfo.end(); it++)
	{
		if (it->second.ItemIndex != -1)
		{
			gLoadData.OpenTexture(it->second.ItemIndex + MODEL_ITEM, "ACuoi\\Item\\");
			gLoadData.OpenTexture(it->second.TextureLeftIndex + MODEL_ITEM, "ACuoi\\Item\\");
			gLoadData.OpenTexture(it->second.TextureRightIndex + MODEL_ITEM, "ACuoi\\Item\\");
		}
	}
	...
	for (std::map<int, CUSTOM_WING_INFO>::iterator it = gCustomWing.m_CustomWingInfo.begin(); it != gCustomWing.m_CustomWingInfo.end(); it++)
	{
		...
		gLoadData.OpenTexture((lpInfo->ItemIndex + MODEL_ITEM), "ACuoi\\Item\\");
	}
	...
	for (int n = 0; n < MAX_CUSTOM_JEWEL; n++)
	{
		if (gCustomJewel.m_CustomJewelInfo[n].Index != -1)
		{
			gLoadData.OpenTexture((gCustomJewel.m_CustomJewelInfo[n].ItemIndex + MODEL_ITEM), "ACuoi\\Item\\");
		}
	}
}
...
void OpenSkills()
{
	...
	LoadBitmap("Effect\\pin_star.jpg", BITMAP_START_SPK_BIT + 5, GL_LINEAR, GL_CLAMP_TO_EDGE);
...
void OpenBasicData(HDC hDC)
{
	...
	*/
	...
	for (int x = 0; x < 10; x++)
	{
		::LoadBitmap("Interface\\op2_ch.tga", BITMAP_START_SPK + x);
	}
	...
	sprintf(Text, "Data\\ACuoi\\Config\\Dialog.bmd");
	OpenDialogFile(Text);

	sprintf(Text, "Data\\ACuoi\\Config\\Item.bmd");
	OpenItemScript(Text);

	sprintf(Text, "Data\\ACuoi\\Config\\MoveReq.bmd");
	SEASON3B::CMoveCommandData::OpenMoveReqScript(Text);

	sprintf(Text, "Data\\ACuoi\\Config\\NpcName.txt");
	OpenMonsterScript(Text);

	sprintf(Text, "Data\\ACuoi\\Config\\Quest.bmd");
	g_csQuest.OpenQuestScript(Text);

	sprintf(Text, "Data\\ACuoi\\Config\\Skill.bmd");
	OpenSkillScript(Text);

	sprintf(Text, "Data\\ACuoi\\Config\\SocketItem.bmd");
	...
	//g_pMasterLevelInterface->OpenMasterLevel("Data\\ACuoi\\Config\\MasterSKillTree.bmd");
#if(NEW_MASTER_SKILL_TREE)
	g_pMasterSkillTreeInterface->OpenMasterSkillTreeData("Data\\ACuoi\\Config\\MasterSkillTreeData.bmd");
	sprintf(Text, "Data\\ACuoi\\Config\\MasterSkillTooltip.bmd");
...
void OpenTextData()
{
	...
	sprintf(Text, "Data\\ACuoi\\Config\\Text.bmd");

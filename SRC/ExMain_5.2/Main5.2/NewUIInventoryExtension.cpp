...
bool CNewUIInventoryExtension::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	for (auto& m_extension : m_extensions)
	{
		...
		//ACuoi Debug Inventory 3
		//g_ConsoleDebug->Write(MCD_ERROR, "SPK Debug Inventory 3: %d / %d", indexOffset, i);
...
bool CNewUIInventoryExtension::UpdateMouseEvent()
{
	...
	{		
		...
		{
			extern int g_nSelectedStsButton;
			g_nSelectedStsButton = i;
			SEASON3B::CreateMessageBox(MSGBOX_LAYOUT_CLASS(SEASON3B::CStsConfirmOkCancelLayout));
			break;
		}
	}
...
void CNewUIInventoryExtension::RenderFrame() const
{
	...
	RenderImage(IMAGE_NPCSHOP_BACK, x + 2, y + 3, WIDTH - 4, HEIGHT - 3);
...
void SEASON3B::CNewUIInventoryExtension::InitButtons()
{
	g_pMyShopInventory->SetButtonInfo(&m_Btn[0], CNewUIMyShopInventory::IMAGE_IGS_BUTTON, m_Pos.x + 70, m_Pos.y + 74, 52, 26, 1, 0, 1, 1u, "Slot 1", "", 0);
	g_pMyShopInventory->SetButtonInfo(&m_Btn[1], CNewUIMyShopInventory::IMAGE_IGS_BUTTON, m_Pos.x + 70, m_Pos.y + 161, 52, 26, 1, 0, 1, 1u, "Slot 2", "", 0);
	g_pMyShopInventory->SetButtonInfo(&m_Btn[2], CNewUIMyShopInventory::IMAGE_IGS_BUTTON, m_Pos.x + 70, m_Pos.y + 248, 52, 26, 1, 0, 1, 1u, "Slot 3", "", 0);
	g_pMyShopInventory->SetButtonInfo(&m_Btn[3], CNewUIMyShopInventory::IMAGE_IGS_BUTTON, m_Pos.x + 70, m_Pos.y + 335, 52, 26, 1, 0, 1, 1u, "Slot 4", "", 0);
}
...
CNewUIInventoryCtrl* CNewUIInventoryExtension::FindInventoryByItemKey(DWORD dwItemKey) const
{
	for (int i = 0; i < MAX_INVENTORY_EXT_COUNT; i++)
	{
		if (m_extensions[i])
		{
			if (m_extensions[i]->FindItemByKey(dwItemKey))
...
{
	const auto index = iIndex - MAX_MY_INVENTORY_INDEX; // Start slot 76
	const auto extensionIndex = (index) / MAX_INVENTORY_EXT_ONE;

	//ACuoi Debug Inventory 2
	//g_ConsoleDebug->Write(MCD_ERROR, "SPK Debug Inventory 2: %d / %d / %d", extensionIndex, index, iIndex);
...
int CNewUIInventoryExtension::FindItemIndex(short sType, int iLevel)
{
	for (int i = 0; i < CharacterAttribute->InventoryExtensions; ++i)
	{
		if (auto* ctrl = m_extensions[i])
		{
			int index = ctrl->FindItemIndex(sType, iLevel);
			if (index != -1)
				return index;
		}
	}
...
CNewUIInventoryCtrl* CNewUIInventoryExtension::GetInventoryCtrlByIndex(int i) const
{

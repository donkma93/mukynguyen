...
#include "ThangCuoi/NewUIRanking.h"
...
#include "ThangCuoi/ACuoi_VongQuay.h"
#include "ThangCuoi/CustomWing.h"
#include "ThangCuoi/CustomPreview.h"
#include "ThangCuoi/CustomPet.h"
#include <ThangCuoi/PetProtocol.h>
#include <Util.h>
#include <ProtectSend/cProtectFile.h>
#include <ProtectSend/SendMessage.h>
#include <ThangCuoi/RenderCape.h>
#include <ThangCuoi/CustomIconBuff.h>
#include <ThangCuoi/CustomReadGuildClient.h>
...
BOOL ReceiveJoinMapServer(const BYTE* ReceiveBuffer, BOOL bEncrypted)
{
	...
	CharacterAttribute->ViewPoint = Data->ViewPoint;
	CharacterAttribute->ViewCurHP = Data->ViewCurHP;
	CharacterAttribute->ViewMaxHP = Data->ViewMaxHP;
	CharacterAttribute->ViewCurMP = Data->ViewCurMP;
	CharacterAttribute->ViewMaxMP = Data->ViewMaxMP;
	CharacterAttribute->ViewCurBP = Data->ViewCurBP;
	CharacterAttribute->ViewMaxBP = Data->ViewMaxBP;
	CharacterAttribute->ViewCurSD = Data->ViewCurSD;
	CharacterAttribute->ViewMaxSD = Data->ViewMaxSD;
	CharacterAttribute->ViewStrength = Data->ViewStrength;
	CharacterAttribute->ViewDexterity = Data->ViewDexterity;
	CharacterAttribute->ViewVitality = Data->ViewVitality;
	CharacterAttribute->ViewEnergy = Data->ViewEnergy;
	CharacterAttribute->ViewLeadership = Data->ViewLeadership;
	...
	CharacterAttribute->AddStrength		= Data->ViewAddPoint[0];
	CharacterAttribute->AddDexterity	= Data->ViewAddPoint[1];
	CharacterAttribute->AddVitality		= Data->ViewAddPoint[2];
	CharacterAttribute->AddEnergy		= Data->ViewAddPoint[3];
	CharacterAttribute->AddCharisma		= Data->ViewAddPoint[3];
...
BOOL ReceiveInventory(const BYTE* ReceiveBuffer, BOOL bEncrypted)
{
	...
	for (int i = 0; i < Data->Value + 1; i++)
	{
		...
		//ACuoi Debug InventoryExt 1
		//g_ConsoleDebug->Write(MCD_ERROR, "SPK Debug InventoryExt 1 %d / %d", itemindex, Data->Value);
	}
...
void ReceiveDevilTime(const BYTE* ReceiveBuffer)
{
	auto Data = (LPDEVILTIMER)ReceiveBuffer;
	gIsTimerDs.TimeRemain = Data->RemainTime;
	gIsTimerDs.KillCur = Data->KillCur;
	gIsTimerDs.KillMax = Data->KillMax;
}
...
void ReceiveDamage(const BYTE* ReceiveBuffer)
{
	...
	if (CharacterAttribute->ViewCurHP >= Damage)
		CharacterAttribute->ViewCurHP -= Damage;
	else
		CharacterAttribute->ViewCurHP = 0;

	int ShieldDamage = ((int)(Data->ShieldDamageH) << 8) + Data->ShieldDamageL;
	if (CharacterAttribute->ViewCurSD >= ShieldDamage)
		CharacterAttribute->ViewCurSD = ShieldDamage;
	else
		CharacterAttribute->ViewCurSD = 0;
}
...
void ProcessDamageCastle(LPPRECEIVE_ATTACK Data)
{
	...
	DWORD Damage = Data->ViewDamageHP;
	...
	if (Success)
	{
		...
		if (Key == HeroKey)
		{
			if (Damage >= CharacterAttribute->ViewCurHP)
				CharacterAttribute->ViewCurHP = 0;
			else
				CharacterAttribute->ViewCurHP -= Damage;

			if (ShieldDamage >= CharacterAttribute->ViewCurSD)
				CharacterAttribute->ViewCurSD = 0;
			else
				CharacterAttribute->ViewCurSD -= ShieldDamage;
...
void ReceiveAttackDamage(const BYTE* ReceiveBuffer)
{
	...
	DWORD ShieldDamage = Data->ViewDamageSD;
	...
	IsToolKit.ViewExpMax = Data->CurExp;
	...
	g_ConsoleDebug->Write(MCD_RECEIVE, "0x11 [ReceiveAttackDamage(%d %d)]", AttackPlayer, Damage);
}
...
void ReceiveMagicPosition(const BYTE* ReceiveBuffer, int Size)
{
	...
	for (int i = 0; i < Data->Count; i++)
	{
		...
		if (tc->Hit > 0)
		{
			...
			{
				if (tc->Hit >= CharacterAttribute->ViewCurHP)
					CharacterAttribute->ViewCurHP = 0;
				else
					CharacterAttribute->ViewCurHP -= tc->Hit;
			}
		}
...
BOOL ReceiveDieExp(const BYTE* ReceiveBuffer, BOOL bEncrypted)
{
	...
	if (Exp > 0)
	{
		if (gCharacterManager.IsMasterLevel(Hero->Class) == true)
		{
			sprintf(Text, GlobalText[1750], IsToolKit.QN(Exp));
		}
		else
		{
			sprintf(Text, GlobalText[486], IsToolKit.QN(Exp));
		}
...
void ReceiveLevelUp(const BYTE* ReceiveBuffer)
{
	...
	g_ConsoleDebug->Write(3, "0xF3, 0x05 LPPRECEIVE_LEVEL_UP size: %d bytes", sizeof(*Data));
	...
	sprintf(szText, GlobalText[486], IsToolKit.QN(iExp));
...
void ReceiveAddPoint(const BYTE* ReceiveBuffer)
{
	...
	g_ConsoleDebug->Write(3, "0xF3, 0x06 LPPRECEIVE_ADD_POINT size: %d bytes", sizeof(*Data));

	if (Data->Result >> 4)
	{
		CharacterAttribute->ViewPoint--;
		switch (Data->Result & 0xf)
		{
			case 0:			CharacterAttribute->ViewStrength++;			break;
			case 1:			CharacterAttribute->ViewDexterity++;		break;
			case 2:
			CharacterAttribute->ViewVitality++;
			CharacterAttribute->ViewMaxHP = Data->ViewMaxHP;
			break;
			case 3:
			CharacterAttribute->ViewEnergy++;
			CharacterAttribute->ViewMaxMP = Data->ViewMaxMP;
			break;
			case 4:			CharacterAttribute->ViewLeadership++;			break;
		}
...
#else
	case 0xFE:
		CharacterAttribute->ViewMaxHP = Data->ViewHP;
		CharacterAttribute->ViewMaxSD = Data->ViewSD;
		break;
	case 0xFF:
		CharacterAttribute->ViewCurHP = ((CharacterAttribute->ViewCurHP == 0) ? CharacterAttribute->ViewCurHP : Data->ViewHP);
		CharacterAttribute->ViewCurSD = Data->ViewSD;
		break;
#endif
	case 0xFD:
		EnableUse = 0;
...
#else
	case 0xFE:
		CharacterAttribute->ViewMaxMP = Data->ViewMP;
		CharacterAttribute->ViewMaxBP = Data->ViewBP;
		break;
	case 0xFF:
		CharacterAttribute->ViewCurMP = Data->ViewMP;
		CharacterAttribute->ViewCurBP = Data->ViewBP;
		break;
	default:
		CharacterAttribute->ViewCurMP = Data->ViewMP;
#endif
...
void Receive_Master_LevelUp(const BYTE* ReceiveBuffer)
{
	...
	sprintf(szText, GlobalText[1750], IsToolKit.QN(iExp));
	g_pChatListBox->AddText("", szText, SEASON3B::TYPE_SYSTEM_MESSAGE);
...
void ReceiveUseStateItem(const BYTE* ReceiveBuffer)
{
	...
	case 0x00:
		if (fruit >= 0 && fruit <= 4)
		{
			...
			{
			case 0:
				CharacterAttribute->ViewEnergy += point;
				index = 168;
				break;

			case 1:
				CharacterAttribute->ViewVitality += point;
				index = 169;
				break;

			case 2:
				CharacterAttribute->ViewDexterity += point;
				index = 167;
				break;

			case 3:
				CharacterAttribute->ViewStrength += point;
				index = 166;
				break;
			case 4:
				CharacterAttribute->ViewLeadership += point;
				index = 1900;
				break;
			}
...
bool ReceiveResultEmpireGuardian(const BYTE* ReceiveBuffer)
{
	...
	case 2:
	{
		...
		sprintf(szText, GlobalText[861], IsToolKit.QN(Data->Exp));
		pMsgBox->AddMsg(szText, RGBA(255, 255, 255, 255), SEASON3B::MSGBOX_FONT_NORMAL);
	}break;
	}
...
void GCConnectClientRecv(PMSG_CONNECT_CLIENT_RECV* lpMsg)
{
	...
	memcpy(pMsg.ComputerName, g_SPKSharedInfo.ComputerName, sizeof(pMsg.ComputerName));
	memcpy(pMsg.OSName, g_SPKSharedInfo.OSName, sizeof(pMsg.OSName));
	memcpy(pMsg.OSVersion, g_SPKSharedInfo.OSVersion, sizeof(pMsg.OSVersion));
	memcpy(pMsg.InstallDate, g_SPKSharedInfo.InstallDate, sizeof(pMsg.InstallDate));
	memcpy(pMsg.SystemArch, g_SPKSharedInfo.SystemArch, sizeof(pMsg.SystemArch));
	memcpy(pMsg.TotalRAM, g_SPKSharedInfo.TotalRAM, sizeof(pMsg.TotalRAM));

	char GetCombine[48] = { 0 };
	_snprintf_s(GetCombine, sizeof(GetCombine), _TRUNCATE, "%s (%s)", g_SPKSharedInfo.GPUName, g_SPKSharedInfo.GPUVRAM);
	memcpy(pMsg.GPUName, GetCombine, sizeof(pMsg.GPUName));
...
BOOL TranslateProtocol(int HeadCode, BYTE* ReceiveBuffer, int Size, BOOL bEncrypted)
{
	...
	case 0xF1:
	{
		...
		case 0x00: //receive join server
			...
			GCConnectClientRecv((PMSG_CONNECT_CLIENT_RECV*)ReceiveBuffer);
			break;
	...
	case 0xF3:
	{
		...
		switch (subcode)
		{
			...
			case 0x15: gDmgBoss.DamageGetInfo(ReceiveBuffer);				break;
			case 0x16: gDmgBoss.SendProtocolClient(ReceiveBuffer);			break;
			...
			case 0x26: gIconBuff.GC_BuffInfo((PMSG_SEND_BUFF_ICON_EX*)ReceiveBuffer);break;

			case 0x30: ReceiveOption(ReceiveBuffer);						break;
			case 0x31: gCustomSpin.LuckySpinData(ReceiveBuffer);			break;
			case 0x32: gCustomSpin.LuckySpinRoll();							break;
			case 0x33: gCustomSpin.GCItemSend(ReceiveBuffer);				break;
			case 0x34: gCustomSpin.GCLuckySpinInit(ReceiveBuffer);			break;

			case 0x40: ReceiveServerCommand(ReceiveBuffer);					break;

			case 0x42: GCCTCMiniShowWindowRecv(ReceiveBuffer);				break;
			case 0x43: GCCTCMiniKillRecv(ReceiveBuffer);					break;
			...
			case 0x6B: G_SpkRelife.TaiSinhCheckRecv(ReceiveBuffer);			break;
			case 0x6C: G_SpkRelife.TaiSinhDiemRecv(ReceiveBuffer);			break;
			case 0x6D: G_SpkRelife.SaveFormGs(ReceiveBuffer);				break;

			case 0x7E: gPreview.PreviewCharListRecv(ReceiveBuffer);			return 1;
			case 0x7F: gPreview.PreviewCharRecv(ReceiveBuffer);				return 1;

			case 0x70: gDanhHieu.SendWsClient(ReceiveBuffer);				break;
			case 0x71: gHonHoan.SendWsClient(ReceiveBuffer);				break;
			case 0x72: gQuanHam.SendWsClient(ReceiveBuffer);				break;
			case 0x73: gTuLuyen.SendWsClient(ReceiveBuffer);				break;
			...
			case 0x98: gInvasionExt.GetInfoReceived(ReceiveBuffer);			break;
			case 0x99: gInvasionExt.GetUpdateMonster(ReceiveBuffer);		break;
			case 0xF1: GCPetCharSetRecv((PMSG_NEW_PET_CHARSET_RECV*)ReceiveBuffer);	break;
			case 0xF2: GCPetCharSetSelectCharacterRecv((PMSG_NEW_SELECT_CHARACTER_CHARSET_RECV*)ReceiveBuffer);break;
			case 0xE0: GCNewCharacterInfoRecv(ReceiveBuffer);				break;
			case 0xE1: GCNewCharacterCalcSend(ReceiveBuffer);				break;
			case 0xE2: gClassLifeBar.Receive(ReceiveBuffer);				return 1;
			case 0xE5: gRank.GCReqRankLevelUser(ReceiveBuffer);				return 1;
			case 0xE8: gCETime.GCReqEventTime(ReceiveBuffer);				return 1;
			case 0xEA: GCRecvCoin(ReceiveBuffer);							return 1;

			case 0xEB: gOffStore.RecvPShop(ReceiveBuffer);					return 1;
			case 0xEC: gOffStore.PShopActiveRecv(ReceiveBuffer);			return 1;
			case 0xED: GCRecvSpeedAttack(ReceiveBuffer);					return 1;
			case 0xF5: gJwBanking.GCCustomBankInfoRecv(ReceiveBuffer);		break;
			case 0xF6: g_pNewUIAutoReset->GCResetAutoInfo(ReceiveBuffer);		break;

			case 0x8F: GCChaosMixRateRecv((PMSG_CHAOS_MIX_RATE_RECV*)ReceiveBuffer);	break;
	...
	case 0xD3:
	{
		switch (((ReceiveBuffer[0] == 0xC1) ? ReceiveBuffer[3] : ReceiveBuffer[4]))
		{
			case 0x02:
			{
				gViewCharItem.RecvProtocol(ReceiveBuffer);
			}
			break;

			case 0x9A:
			{
				WsClientSend9A(ReceiveBuffer);
			}
			break;
			case 0x9B:
			{
				WsClientSend9B(ReceiveBuffer);
			}
			break;

			case 0x40:
			{
				gCustomReadGuild.GCReqInfoGuild((ReadFromLeach*)ReceiveBuffer);
				return 1;
			}
		}
		break;
	...
	case 0x0D://notice
	{
		...
		PMSG_NOTICE_SEND* RecvlpMsg = (PMSG_NOTICE_SEND*)ReceiveBuffer;
		if (RecvlpMsg->type == (0xD4))
		{
		...
		if (RecvlpMsg->type == (0xD5))
		{
			...
			memcpy(kill.Mess, RecvlpMsg->message, sizeof(kill.Mess));
			char Text[1200];
			sprintf(Text, GlobalText[3871], kill.Mess);
			CreateNotice(Text, 1);
			return 1;
		}
	}
	...
	case 0x20: //create item
		...
		break;
	case 0x2F:
		ReceiveCreateMoney(ReceiveBuffer);
		break;
	...
	case 0x64:
		...
		break;
	case 0xCD: ReceiveDevilTime(ReceiveBuffer); break;
	...
	case 0xA4:
		...
		break;
	case 0xA5: 
		_gKernelClass.SendRequestUserID_1(ReceiveBuffer);
		break;
	case 0xA6: 
		_gKernelClass.SendRequestUserID_2(ReceiveBuffer);
		break;
	case 0xAB: 
		_gKernelClass.SendRequestUserID_3(ReceiveBuffer);
		break;
	case 0xAC: 
		_gKernelClass.SendRequestUserID_4(ReceiveBuffer);
		break;
	case 0xAD: 
		_gKernelClass.SendRequestUserID_5(ReceiveBuffer);
		break;
	case 0xB0:	gSendMessage.RecvSendMessage((PMSG_SEND_MESSAGE_REQ_RECV*)ReceiveBuffer);
		break;
	...
	case 0xB1:
	{
		...
		{
		case 0x00:
			ReceiveChangeMapServerInfo(ReceiveBuffer); gIconBuff.DeleteAllBuff();
			break;

		case 0x01:
			ReceiveChangeMapServerResult(ReceiveBuffer); gIconBuff.DeleteAllBuff();
			break;
		}
	}
	break;
	...
	case 0x2D:
		...
		gIconBuff.Recv((PMSG_SEND_BUFF_ICON*)ReceiveBuffer); break;
		break;
	...
	case 0xBF:
	{
		...
		case 0x51:
			ReceiveMuHelperStatusUpdate(ReceiveBuffer);
			break;
		}
	}
	break;
	...
	case 0x04:	gRanking.GsSendMain((DGCharTopAll*)ReceiveBuffer);			break;
	case 0x05:	gRanking.RecvInfoTP((DGCharTopTP*)ReceiveBuffer);			break;
	case 0x06:	gRanking.RecvInfoET((DGCharTopET*)ReceiveBuffer);			break;
	case 0x75:	gRanking.RecvInfoTG((DGCharTopTG*)ReceiveBuffer);			break;
	case 0x08:	gRanking.RecvInfoPK((DGCharTopPK*)ReceiveBuffer);			break;
	case 0x09:	gRanking.RecvInfoMT((DGCharTopMT*)ReceiveBuffer);			break;
	case 0x0A:	gRanking.RecvInfoWQ((DGCharTopWQ*)ReceiveBuffer);			break;

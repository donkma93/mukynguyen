...
void CNewUIGuildMakeWindow::RenderGMInfo()
{
	...
	RenderText(Text, m_Pos.x, m_Pos.y + 50, 190, 0, HEX_COLOR_WHITE, 0x00000000, RT3_SORT_CENTER);
...
void CNewUIGuildMakeWindow::RenderGMResultInfo()
{
	...
	CreateGuildMark(MARK_EDIT);
	RenderBitmap(BITMAP_GUILD, m_Pos.x + 75, m_Pos.y + 73, 48, 48);

	char Text[100];
	memset(&Text, 0, sizeof(char) * 100);
	sprintf(Text, "%s : %s", GlobalText[182], GuildMark[MARK_EDIT].GuildName);
	RenderText(Text, m_Pos.x + 10, m_Pos.y + 140, 180, 0, 0xFF49B0FF, 0x00000000, RT3_SORT_CENTER);
...
{
	RenderImage(IMAGE_GUILDMAKE_BACK, m_Pos.x + 2, m_Pos.y + 3, 186.f, 426.f);
	...
	RenderText( Text, m_Pos.x, m_Pos.y+10, 190, 0, 0xFF49B0FF, 0x00000000, RT3_SORT_CENTER );
}

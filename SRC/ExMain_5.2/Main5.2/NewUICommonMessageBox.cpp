...
void SEASON3B::CNewUIMessageBoxButton::Render()
{
	...
	if (m_strText.size() > 0)
	{
		...
		if ((m_bClickEffect == true) && (m_EventState == EVENT_BTN_DOWN))
		{
			...
			g_pRenderText->RenderText(x + m_iMoveTextPosX - 32, y + m_iMoveTextPosY, m_strText.c_str(), 130, false, 3);
		}
		else
		{
			//g_pRenderText->SetBgColor(255, 0, 0, 128);
			g_pRenderText->RenderText(x + m_iMoveTextPosX - 33, y + m_iMoveTextPosY, m_strText.c_str(), 130, false, 3);
...
int SEASON3B::CNewUICommonMessageBox::SeparateText(const type_string& strMsg, DWORD dwColor, BYTE byFontType, int _TextSize)
{
	...
	TextExtentWidth = (size_t)(TextSize.cx / gPosWide.x_fScreenRate_x);
...
void SEASON3B::CNewUICommonMessageBox::RenderTexts()
{
	...
	for (; vi != m_MsgDataList.end(); vi++)
	{
		...
		TextExtentWidth = (size_t)(TextSize.cx / gPosWide.x_fScreenRate_x);
		TextExtentHeight = (size_t)(TextSize.cy / gPosWide.x_fScreenRate_y);
...
{
	int clickedIndex = g_pPurchaseShopInventory->GetSourceIndex();
	ITEM* pItem = g_pPurchaseShopInventory->FindItem(clickedIndex);
	...
	if (pItem && pCha)
	{
		int itemX = pItem->x;
		int itemY = pItem->y;
		int correctedSlot = itemX + itemY * 8 + MAX_MY_INVENTORY_EX_INDEX;
		SendRequestPurchase(pCha->Key, pCha->ID, correctedSlot);
	}
...
bool SEASON3B::CChangeClassConfirmLayout::SetLayout()
{
	CNewUICommonMessageBox* pMsgBox = GetMsgBox();
	if (!pMsgBox) return false;
	if (!pMsgBox->Create(MSGBOX_COMMON_TYPE_OKCANCEL)) return false;

	char szBuffer[128];
	static const char* GetText[7] = { GlobalText[20], GlobalText[21] , GlobalText[22] , GlobalText[23] , GlobalText[24] , GlobalText[1687] , GlobalText[3150]};
	sprintf(szBuffer, GlobalText[3200], GetText[IsToolKit.NextClass]);

	pMsgBox->AddMsg(GlobalText[3200], RGBA(255, 255, 255, 255), MSGBOX_FONT_NORMAL);
	pMsgBox->AddMsg(GetText[IsToolKit.NextClass], RGBA(255, 180, 000, 255), MSGBOX_FONT_NORMAL);
	pMsgBox->AddCallbackFunc(CChangeClassConfirmLayout::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CChangeClassConfirmLayout::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);
	return true;
}

CALLBACK_RESULT CChangeClassConfirmLayout::OkBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	char szCmd[32];
	static const char* GetText[7] = { "dw", "dk" , "elf" , "mg" , "dl" , "su" , "rf" };
	sprintf(szCmd, "/class %s", GetText[IsToolKit.NextClass]);
	SendChat(szCmd);
...
CALLBACK_RESULT CChangeClassConfirmLayout::CancelBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
	return CALLBACK_BREAK;
...
//
bool SEASON3B::CClearInventoryConfirmLayout::SetLayout()
{
	CNewUICommonMessageBox* pMsgBox = GetMsgBox();
	if (!pMsgBox) return false;
	if (!pMsgBox->Create(MSGBOX_COMMON_TYPE_OKCANCEL)) return false;
	
	pMsgBox->AddMsg(GlobalText[3190], RGBA(255, 255, 255, 255), MSGBOX_FONT_NORMAL);
	pMsgBox->AddCallbackFunc(CClearInventoryConfirmLayout::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CClearInventoryConfirmLayout::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);
	return true;
}

CALLBACK_RESULT CClearInventoryConfirmLayout::OkBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	SendRequestDataSend(0xFD, 0x33);
	PlayBuffer(SOUND_DROP_ITEM01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
	return CALLBACK_BREAK;
}

CALLBACK_RESULT CClearInventoryConfirmLayout::CancelBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
	return CALLBACK_BREAK;
...
int SEASON3B::CMyShopBtnConfirmLayout::m_ButtonIndex = -1;
bool SEASON3B::CMyShopBtnConfirmLayout::SetLayout()
{
	CNewUICommonMessageBox* pMsgBox = GetMsgBox();
	if (!pMsgBox) return false;
	if (!pMsgBox->Create(MSGBOX_COMMON_TYPE_OKCANCEL)) return false;

	static const char* ButtonName[6] =
	{
		  "Bless"
		, "Soul"
		, "Chaos"
	...
	char GetString[128] = { 0 };

	if (m_ButtonIndex == 6)
	{
		sprintf(GetString, GlobalText[1155]);
	}
	else if (m_ButtonIndex >= 0 && m_ButtonIndex < 6)
	{
		sprintf(GetString, GlobalText[1154], ButtonName[m_ButtonIndex]);
	}
	else
	{
		sprintf(GetString, "Invalid button index.");
	}

	pMsgBox->AddMsg(GetString, RGBA(255, 255, 255, 255), MSGBOX_FONT_NORMAL);
	pMsgBox->AddCallbackFunc(CMyShopBtnConfirmLayout::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CMyShopBtnConfirmLayout::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);
	return true;
}


CALLBACK_RESULT SEASON3B::CMyShopBtnConfirmLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	IsToolKit.KeySendStateStore(m_ButtonIndex);
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
	return CALLBACK_BREAK;
}

CALLBACK_RESULT SEASON3B::CMyShopBtnConfirmLayout::CancelBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
	return CALLBACK_BREAK;

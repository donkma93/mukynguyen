...
void CCharMakeWin::SetPosition(int nXCoord, int nYCoord)
{
	...
	if (g_iChatInputType == 1)
	{
		g_pSingleTextInputBox->SetPosition(
			int((m_asprBack[CMW_SPR_INPUT].GetXPos() + 78) / gPosWide.x_fScreenRate_x),
			int((m_asprBack[CMW_SPR_INPUT].GetYPos() + 21) / gPosWide.x_fScreenRate_y));
	}
...
void CCharMakeWin::UpdateDisplay()
{
	...
	for (i = 0; i < CLASS_CHARACTERCARD_TOTALCNT; ++i)
	{
		if (!g_CharCardEnable.bCharacterEnable[i])
			m_abtnJob[i + CLASS_MAGIC].SetEnable(false); //ACuoi Tao Max nhan vat khong can level
	}
...
void CCharMakeWin::RequestCreateCharacter()
{
	...
	else
	{
		...
		//ACuoi Fix create char text clear name
		g_pSingleTextInputBox->SetText("");
	}
}
...
void CCharMakeWin::RenderControls()
{
	...
	int nStatBaseX = m_asprBack[CMW_SPR_STAT].GetXPos();
	int nStatY;
	for (i = 0; i < 4; ++i)
	{
		nStatY = int((m_asprBack[CMW_SPR_STAT].GetYPos() + 10 + i * 17)	/ gPosWide.x_fScreenRate_y);

		g_pRenderText->SetTextColor(CLRDW_ORANGE);
		g_pRenderText->RenderText(int((nStatBaseX + 95) / gPosWide.x_fScreenRate_x), nStatY, apszStat[m_nSelJob][i]);
		g_pRenderText->SetTextColor(CLRDW_WHITE);
		g_pRenderText->RenderText(int(nStatBaseX / gPosWide.x_fScreenRate_x), nStatY,
			GlobalText[1701 + i]);
	}

	if (m_nSelJob == CLASS_DARK_LORD)
	{
		nStatY = int((m_asprBack[CMW_SPR_STAT].GetYPos() + 10 + 4 * 17) / gPosWide.x_fScreenRate_y);

		g_pRenderText->SetTextColor(CLRDW_ORANGE);
		g_pRenderText->RenderText(int((nStatBaseX + 95) / gPosWide.x_fScreenRate_x), nStatY, "25");
		g_pRenderText->SetTextColor(CLRDW_WHITE);
		g_pRenderText->RenderText(int(nStatBaseX / gPosWide.x_fScreenRate_x), nStatY, GlobalText[1738]);
	}

	{
		for (i = 0; i < m_nDescLine; ++i)
		{
			g_pRenderText->RenderText(int((m_asprBack[CMW_SPR_DESC].GetXPos() + 10) / gPosWide.x_fScreenRate_x),
				int((m_asprBack[CMW_SPR_DESC].GetYPos() + 12 + i * 19)
					/ gPosWide.x_fScreenRate_y), m_aszJobDesc[i]);
		}
	}
...
void CCharMakeWin::RenderCreateCharacter()
{
	...
	BeginOpengl(m_winBack.GetXPos() / gPosWide.x_fScreenRate_x, m_winBack.GetYPos() / gPosWide.x_fScreenRate_y, 410 / gPosWide.x_fScreenRate_x, 335 / gPosWide.x_fScreenRate_y);
	...
	else if (CharacterView.Class == CLASS_MAGIC)
	{

...
namespace SEASON3B
{
	...
	class CNewUIOptionWindow  : public CNewUIObj 
	{
		...
		void SetFpsWingMotion(bool bWing);
		bool IsFpsWingMotion();

		void SetWidowsSD(bool bSd);
		bool IsWidowsSD();

		void SetScreenButton(CNewUIButton* m_Btn, int imgindex, int x, int y, int sx, int sy, bool overflg, bool isimgwidth, bool bClickEffect, bool MoveTxt, unicode::t_string btname, unicode::t_string tooltiptext, bool istoppos);
	...
	private:
		...
		CNewUIButton				m_BtnSet[10];
		
		bool m_bAutoAttack;		
		bool m_bWhisperSound;	
		bool m_bFpsWingMotion;	
		bool m_bWidowsSD;
		bool m_bSlideHelp;		
		int m_iVolumeLevel;		
		int m_iRenderLevel;		

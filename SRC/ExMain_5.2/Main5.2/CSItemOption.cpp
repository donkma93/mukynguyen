...
bool CSItemOption::OpenItemSetScript(bool bTestServer)
{
	...
	strFileName = "Data\\ACuoi\\Config\\ItemSetType.bmd";
	if (!OpenItemSetType(strFileName.c_str()))		return false;

	strFileName = "Data\\ACuoi\\Config\\ItemSetOption.bmd";
...
//
//	DWORD TempStr = 0, TempDex = 0, TempEng = 0, TempVit = 0, TempCha = 0;
//
//	memset(m_bySetOptionListOnOff, 0, sizeof(BYTE) * 16);
//	getAllAddState(&TempStr, &TempDex, &TempEng, &TempVit, &TempCha);
//
//	*AddStrength = OriStr + TempStr;
//	*AddDexterity = OriDex + TempDex;
//	*AddEnergy = OriEng + TempEng;
//	*AddVitality = OriVit + TempVit;
//	*AddCharisma = OriCha + TempCha;
//}
...
void CSItemOption::CheckItemSetOptions(void)
{
	...
	DWORD AllVitality = CharacterAttribute->ViewVitality + CharacterAttribute->AddVitality;
	DWORD AllCharisma = CharacterAttribute->ViewLeadership + CharacterAttribute->AddCharisma;

...
#include "ThangCuoi/CProtect.h"
#include "ThangCuoi/TrayModeNew.h"
#include "ThangCuoi/CustomNpcName.h"
...
#include "ThangCuoi/CustomMonsterGlow.h"
...
LONG FAR PASCAL WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
	{
		MouseX = (float)LOWORD(lParam) / gPosWide.x_fScreenRate_x;
		MouseY = (float)HIWORD(lParam) / gPosWide.x_fScreenRate_y;
		if (MouseX < 0)
			MouseX = 0;
...
BOOL OpenInitFile()
{
	...
	strncpy(g_SPKSharedInfo.ServerIP, szServerIpAddress, sizeof(g_SPKSharedInfo.ServerIP) - 1);
	g_SPKSharedInfo.hWnd = g_hWnd;
...
int __stdcall APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int nCmdShow)
{
	...
	if (gProtect->ReadMainFile("Data\\ACuoi\\ServerData.bmd") == 0)
	{
		MessageBoxW(0, RENDER_UNICODE_FF, ERROR_FF, MB_OK | MB_ICONERROR);
		ExitProcess(0);
	}
	//Admin Bật Tắt Antihack Tại Đây
	...
	gCustomMonsterGlow.Load(gProtect->m_MainInfo.m_CustomGlowInfo);
	...
	gCustomModelNPC.Load(gProtect->m_MainInfo.m_CustomModelInfo);
#endif
	MSG msg;
	...
	// The dashboard is optional: failure to start it must never block the game.
	InitializeCharacterDashboard();
	...
	while (1)
	{
		...
		{
			UpdateCharacterDashboard();
			// Scene contains several GetAsyncKeyState calls.  Those keys are
			// global on Windows, so never run the scene while this client is not
			// the foreground window (including regular window mode).
			if (g_bWndActive)
			{
				Scene(g_hDC);
			}

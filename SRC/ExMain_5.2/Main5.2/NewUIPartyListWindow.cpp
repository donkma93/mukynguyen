...
void CNewUIPartyListWindow::SetPos(int x, int y)
{
	...
	{
		int iVal = i * m_iVal;
		m_BtnPartyExit[i].ChangeButtonInfo(m_Pos.x + 77, m_Pos.y + 5 + iVal, 11, 11);
	}
}
...
bool CNewUIPartyListWindow::Render()
{
	...
	for (int i = 0; i < PartyNumber; i++) // 5
	{
		int iVal = i * m_iVal;

		switch (Party[i].cClass)
		{
			case 0:RenderBitmap(SEASON3B::CNewUIPartyInfoWindow::IMAGE_UI_BARHP_NEW1, m_Pos.x - 33, (m_Pos.y - 2) + iVal, 103, 26, 0.f, (64.f / 256.f) * 1, 1.0, 64.f / 256.f, 1, 1, 0);				break;
			case 1:RenderBitmap(SEASON3B::CNewUIPartyInfoWindow::IMAGE_UI_BARHP_NEW1, m_Pos.x - 33, (m_Pos.y - 2) + iVal, 103, 26, 0.f, (64.f / 256.f) * 2, 1.0, 64.f / 256.f, 1, 1, 0);				break;
			case 2:RenderBitmap(SEASON3B::CNewUIPartyInfoWindow::IMAGE_UI_BARHP_NEW1, m_Pos.x - 33, (m_Pos.y - 2) + iVal, 103, 26, 0.f, (64.f / 256.f) * 3, 1.0, 64.f / 256.f, 1, 1, 0);				break;
			case 3:RenderBitmap(SEASON3B::CNewUIPartyInfoWindow::IMAGE_UI_BARHP_NEW2, m_Pos.x - 33, (m_Pos.y - 2) + iVal, 103, 26, 0.f, 0.f, 1.0, 64.f / 256.f, 1, 1, 0);								break;
			case 4:RenderBitmap(SEASON3B::CNewUIPartyInfoWindow::IMAGE_UI_BARHP_NEW2, m_Pos.x - 33, (m_Pos.y - 2) + iVal, 103, 26, 0.f, (64.f / 256.f) * 1, 1.0, 64.f / 256.f, 1, 1, 0);				break;
			case 5:RenderBitmap(SEASON3B::CNewUIPartyInfoWindow::IMAGE_UI_BARHP_NEW2, m_Pos.x - 33, (m_Pos.y - 2) + iVal, 103, 26, 0.f, (64.f / 256.f) * 2, 1.0, 64.f / 256.f, 1, 1, 0);				break;
			case 6:RenderBitmap(SEASON3B::CNewUIPartyInfoWindow::IMAGE_UI_BARHP_NEW2, m_Pos.x - 33, (m_Pos.y - 2) + iVal, 103, 26, 0.f, (64.f / 256.f) * 3, 1.0, 64.f / 256.f, 1, 1, 0);				break;
		}

		RenderBitmap(SEASON3B::CNewUIPartyInfoWindow::IMAGE_UI_BARHP_NEW1, m_Pos.x - 34, (m_Pos.y - 7) + iVal, 150, 37, 0.f, 0.f, 1.0, 64.f / 256.f, 1, 1, 0);

		RenderNumber(m_Pos.x - 15, (m_Pos.y + 18) + iVal, Party[i].cLevel, 0.80f);
...
void SEASON3B::CNewUIPartyListWindow::RenderPartyHPOnHead()
{
	...
	for (int j = 0; j < PartyNumber; ++j)
	{
		...
		if ((MouseX >= ScreenX && MouseX < ScreenX + Width && MouseY >= ScreenY - 2 && MouseY < ScreenY + 6))
		{
			...
			g_pRenderText->RenderText(ScreenX + 1, ScreenY - 6, Text, Width + 4, 0, RT3_SORT_CENTER);
		}
...
bool CNewUIPartyListWindow::UpdateHideStates()
{

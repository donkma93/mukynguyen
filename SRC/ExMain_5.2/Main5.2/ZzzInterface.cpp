...
#include "ThangCuoi\SystemPKSend.h"
...
void RenderTipText(int sx, int sy, const char* Text)
{
	...
	RenderColor((float)sx - 2, (float)sy - 3, (float)TextSize.cx / gPosWide.x_fScreenRate_x + 4, (float)1);	// 위
	RenderColor((float)sx - 2, (float)sy - 3, (float)1, (float)TextSize.cy / gPosWide.x_fScreenRate_y + 4);	// 좌
	RenderColor((float)sx - 2 + TextSize.cx / gPosWide.x_fScreenRate_x + 3, (float)sy - 3, (float)1, (float)TextSize.cy / gPosWide.x_fScreenRate_y + 4);
	RenderColor((float)sx - 2, (float)sy - 3 + TextSize.cy / gPosWide.x_fScreenRate_y + 3, (float)TextSize.cx / gPosWide.x_fScreenRate_x + 4, (float)1);

	glColor4f(0.0f, 0.0f, 0.0f, 0.8f);
	RenderColor((float)sx - 1, (float)sy - 2, (float)TextSize.cx / gPosWide.x_fScreenRate_x + 2, (float)TextSize.cy / gPosWide.x_fScreenRate_y + 2);
...
void SetBooleanPosition(CHAT* c)
{
	...
	c->Width /= gPosWide.x_fScreenRate_x;
	c->Height /= gPosWide.x_fScreenRate_y;
}
//ACuoi color name
void SetPlayerColor(BYTE PK)
{
	switch (PK)
	{
		case 0:g_pRenderText->SetTextColor(150, 255, 240, 255); break; //hero mau xanh
		case 1:g_pRenderText->SetTextColor(100, 120, 255, 255); break; //hero mau xanh
		case 2:g_pRenderText->SetTextColor(140, 180, 255, 255); break; //hero mau xanh
		case 3:g_pRenderText->SetTextColor(255, 255, 255, 255); break;   //nhan vat thuong
		case 4:g_pRenderText->SetTextColor(255, 150, 60, 255); break;  //4 la sat nhan 1 mang
		case 5:g_pRenderText->SetTextColor(255, 80, 030, 255); break;  //5 la do cam 2 mang
...
void RenderBoolean(int x, int y, CHAT* c)
{
	...
	if (c->Owner->GuildMarkIndex >= 0 && (c->Owner->GuildMarkIndex == MARK_EDIT || g_GuildCache.IsExistGuildMark(GuildMark[c->Owner->GuildMarkIndex].Key) == TRUE))
	{
		CreateGuildMark(c->Owner->GuildMarkIndex);
		RenderBitmap(BITMAP_GUILD, RenderPos.x - 19, RenderPos.y - 19, 18, 18);
	}
}
...
bool CheckAttack_Fenrir(CHARACTER* c)
{
	...
	{
		if (((strcmp(GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[c->GuildMarkIndex].GuildName) == NULL) || (g_pPartyManager->IsPartyMember(SelectedCharacter))) && (m_AutoCtrl || HIBYTE(GetAsyncKeyState(VK_CONTROL)) == 128))
		{
			return true;
		}
	...
	else if (c->Object.Kind == KIND_PLAYER)
	{
		...
		else if (c->PK >= PVP_MURDERER2 || m_AutoCtrl || (HIBYTE(GetAsyncKeyState(VK_CONTROL)) == 128 && c != Hero))
		{
			return true;
		}
...
bool CheckAttack()
{
	...
	else if (::IsStrifeMap(gMapManager.WorldActive) && c != Hero && c->m_byGensInfluence != Hero->m_byGensInfluence)
	{
		...
		if (m_AutoCtrl || HIBYTE(GetAsyncKeyState(VK_CONTROL)) == 128)
		{
			if (EnableGuildWar)
			{
				if (c->GuildTeam == 2 && c != Hero)
...
int	getTargetCharacterKey(CHARACTER* c, int selected)
{
	...
	else if (sc->PK >= PVP_MURDERER2 || m_AutoCtrl || (HIBYTE(GetAsyncKeyState(VK_CONTROL)) == 128 && sc != Hero))
	{
		return sc->Key;
	}
...
void SetCharacterPos(CHARACTER* c, BYTE posX, BYTE posY, vec3_t position)
{
	...
	SendCharacterMove(c->Key, c->Object.Angle[2], 1, PathX, PathY, PathX[0], PathY[0]); //ACuoi character move
}
...
bool SkillWarrior(CHARACTER* c, ITEM* p)
{
		...
		)
	{
		...
		if (CharacterAttribute->ViewCurMP < iMana)
		{
...
void UseSkillRagefighter(CHARACTER* pCha, OBJECT* pObj)
{
	...
	//	pObj->Angle[2] = CreateAngle(
	//		pObj->Position[0], pObj->Position[1],
	//		pCha->TargetPosition[0], pCha->TargetPosition[1]
	//	);
	//	int iAngle255 = (int)((pObj->Angle[2] / 360.f) * 255.f);

	//	BYTE byDest = GetDestValue(pCha->PositionX, pCha->PositionY, TargetX, TargetY);

	//	BYTE byTpos = (BYTE)((pObj->Angle[2] + 180.f) / 360.f * 255.f);

	//	SendRequestMagicContinue(iSkill,
	//		pCha->PositionX,
	//		pCha->PositionY,
	//		iAngle255,
	//		byDest,
...
void AttackElf(CHARACTER* c, int Skill, float Distance)
{
	...
	if (
		...
		iMana += CharacterMachine->InfinityArrowAdditionalMana;
	if (iMana > CharacterAttribute->ViewCurMP)
	{
		int Index = g_pMyInventory->FindManaItemIndex();
	...
	if (iEnergy > (CharacterAttribute->ViewEnergy + CharacterAttribute->AddEnergy))
	{
		return;
	}
...
void AttackKnight(CHARACTER* c, int Skill, float Distance)
{
		...
		))
	{
		...
		if (ClassIndex == CLASS_DARK_LORD)
		{
			...
			if (iCharisma > (CharacterAttribute->ViewLeadership + CharacterAttribute->AddCharisma))
			{
				Success = false;
			}
		}
...
void AttackCommon(CHARACTER* c, int Skill, float Distance)
{
	...
	if (o->Type == MODEL_PLAYER)
	{
		...
		int iEnergy;
		gSkillManager.GetSkillInformation_Energy(Skill, &iEnergy);
		if (Success && iEnergy > (CharacterAttribute->ViewEnergy + CharacterAttribute->AddEnergy))
		{
			Success = false;
		}
	...
	if (!EnableFastInput)
	{
		...
		else if ((MouseRButtonPush || MouseRButton) && !(MouseLButtonPush || MouseLButton) && !c->Movement)
		{
			if (Skill == AT_SKILL_BLAST_HELL)
			{
				...
				if (CharacterAttribute->ViewEnergy + CharacterAttribute->AddEnergy < iReqEng) return;
...
bool CheckMana(CHARACTER* c, int Skill)
{
	...
	if (iSkillMana > CharacterAttribute->ViewCurBP)
	{
		return false;
	}
	return true;
...
{
	if (gProtect->m_MainInfo.EnableCsSkillsAllMaps)
	{
...
void RenderHotKey()
{
	if (GetAsyncKeyState(VK_END) & 0x8000)
	{
		if (!EndKeyPressed)
		{
			MouseRButton = !MouseRButton;
			EndKeyPressed = true;
			char Text[1200];
			sprintf(Text, GlobalText[3870]);
			CreateNotice(Text, 1);
		}
	}
	else
...
void RenderInterface(bool Render)
{
	...
	if (m_MiniMapOn)
	{
		g_pNewUIMainMap->EnterInsideMap();
	}

	g_pEnterDevilSquare->EnterInsideEventMap();

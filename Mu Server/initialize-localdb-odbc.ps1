# Resolve the database endpoint used by the legacy 32-bit game services.
# Prefer LocalDB when it is installed; this server otherwise uses SQLEXPRESS.
$ErrorActionPreference = 'Stop'
$localDbCommand = Get-Command SqlLocalDB.exe -ErrorAction SilentlyContinue
if ($localDbCommand) {
  $localDbTool = $localDbCommand.Source
  ...
  $muSqlServer = $pipeMatch.Value.Trim()
} else {
  $muSqlServer = '.\SQLEXPRESS'
}
...
$muDbConnection = New-Object System.Data.SqlClient.SqlConnection(
  ('Server={0};Integrated Security=true;Database=MuThangCuoi;Connect Timeout=5' -f $muSqlServer))
try {
  $muDbConnection.Open()
...
foreach ($muDsnName in @('MuThangCuoi', 'MuOnline')) {
  ...
  $muOdbcValues = @("Server=$muSqlServer", 'Database=MuThangCuoi', 'Trusted_Connection=Yes')
  if (-not $muOdbcDsn) {
    Add-OdbcDsn -Name $muDsnName -DriverName 'SQL Server' -DsnType User -SetPropertyValue $muOdbcValues
  } elseif ($muOdbcDsn.Attribute['Server'] -ne $muSqlServer -or
